describe('Scenario Parsing', function() {
    ...
    it('should parse Scenario Outlines from Features', function(done) {
        ...
        framework.match(__dirname, function(specs) {
            ...
            featureScenarios[0].getExpectations()[0].getTitle().should.be.exactly('Given the preset PA');
            featureScenarios[0].getExpectations()[1].getTitle().should.be.exactly('When the condition CA');
            featureScenarios[0].getExpectations()[2].getTitle().should.be.exactly('Then perform the action AA');

            featureScenarios[1].getExpectations()[0].getTitle().should.be.exactly('Given the preset PB');
            featureScenarios[1].getExpectations()[1].getTitle().should.be.exactly('When the condition CB');
            featureScenarios[1].getExpectations()[2].getTitle().should.be.exactly('Then perform the action AB');

            // Check Feature Scenario Expectation Locations
            featureScenarios[0].getExpectations()[0].getLocation().should.be.eql({
                filename: __dirname + '/features/a.feature',
                col: 8,
                line: 6
            });

            featureScenarios[1].getExpectations()[0].getLocation().should.be.eql({
                filename: __dirname + '/features/a.feature',
                col: 8,
                line: 6
            });
