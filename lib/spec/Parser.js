...
function Parser(options) {
    ...
    var Framework;
    try {
        Framework = require(path.join(__dirname, '..', 'framework', options.tests.framework + '.js'));
...
Parser.parseFeatureFromPatterns = function(patterns) {
    ...
    patterns.forEach(function(pattern) {
        glob.sync(pattern).forEach(function(file) {
            /* istanbul ignore else: TODO Test with multiple overlapping glob patterns? */
...
Parser.prototype = {
    ...
    _getSpecsFromFiles: function(files, options, framework) {
        ...
        if (typeof options.specs.matching === 'function') {
            matcher = options.specs.matching;

        } else if (typeof options.specs.matching.type === 'string') {
            matcher = Matchers[options.specs.matching.type];
        }
