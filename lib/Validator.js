...
Validator.prototype = {
        ...
        for(var feature in errors) {
            if (errors.hasOwnProperty(feature)) {
                ...
                for(var scenario in errors[feature]) {
                    if (errors[feature].hasOwnProperty(scenario) && scenario !== '') {
                        ...
                        if (scenarioErrors.length) {
                            ...
                            featureLines.push(util.color('    Scenario: ', 'green') + util.color(scenario, 'white'));
                        }
                ...
                if (featureErrors.length || scenarioErrorCount) {
                    ...
                    lines.push(util.color('Feature: ', 'green') + util.color(feature, 'white'));
                }
    ...
    _compareSpec: function(spec, errors) {
        ...
        // TODO correct error ordering and improve printout on a per file scenario level
    ...
    _compareFeatureScenarios: function(topErrors, feature, test, distance) {
        ...
        this._compareTags('Feature tags mismatch:', feature, test, errors['']);

        // Compare Feature Title
        if (distance !== 0) {
            errors[''].push({
                message: 'Feature title mismatch:',
                test: test,
                feature: feature
            });
        }
        ...
        if (feature.getDescription() !== test.getDescription()) {
            errors[''].push({
                message: 'Feature description mismatch:',
                description: true,
                test: test,
                feature: feature
            });
    ...
    _compareScenarioExpecations: function(topErrors, feature, test, distance) {
        ...
        if (distance !== 0) {
            errors.push({
                message: 'Scenario title mismatch:',
                test: test,
                feature: feature,
        ...
        this._compareTags('Scenario tags mismatch:', feature, test, errors);
    ...
    _compareAssertion: function(errors, feature, test, distance) {
        ...
        if (distance > 0) {
            errors.push({
                message: 'Expectation title mismatch:',
                test: test,
                feature: feature,
                loc: feature.getLocation()
            });
        ...
        if (!test.hasExpression()) {
            errors.push({
                message: 'Missing test code for Expectation:',
                title: test.getTitle(),
                test: test,
                feature: null
            });
    ...
    _formatError: function(error) {
        ...
        if (error.test && error.feature) {
            ...
            locationTwo = this._formatLocation(error.feature.getLocation());
    ...
    _formatLocation: function(loc, noPrefix) {
        ...
        if (!noPrefix) {
            ...
            msg = util.color(loc.filename, util.color.Key);
        }
