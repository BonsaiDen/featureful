                ...
                case keywords.And:
                    if (conditions.length === 0) {
                        throw new Error(
                            ...
                            + ': Expected a Given/Then/When before AND in scenario steps.'
                        );
        ...
        for(var i in templateSteps) {
            if (templateSteps.hasOwnProperty(i)) {
                steps[i] = templateSteps[i].map(function(node) {
                    return this.replaceNode(node, headers, map, cols);
...
function GivenNode(keywords, title, data, and, but, loc, index) {
    ...
    this.title = dedupeTitlePrefix(title.trim(), this.keywords.Given);
    this.data = data;
    this.and = and;
    this.but = but;
    this.loc = loc;
...
function WhenNode(keywords, title, data, and, but, loc, index) {
    this.type = 'WHEN';
    this.keywords = keywords;
    this.title = dedupeTitlePrefix(title.trim(), this.keywords.When);
    this.data = data;
    this.and = and;
    this.but = but;
    this.loc = loc;
...
function ThenNode(keywords, title, data, and, but, loc, index) {
    this.type = 'THEN';
    this.keywords = keywords;
    this.title = dedupeTitlePrefix(title.trim(), this.keywords.Then);
    this.data = data;
    this.and = and;
    this.but = but;
    this.loc = loc;
