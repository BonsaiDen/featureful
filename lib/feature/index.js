...
Parser.prototype = {
    ...
    scenarioFromTemplate: function(index, tags, description, templateSteps, headers, map, rows, location) {
        ...
        return new ScenarioNode(
            tags, description + ' (' + this._keywords.Example + ' #' + index + ')',
            steps, null, location
        );
    ...
    replaceNode: function(node, headers, map, rows) {
        ...
        if (node instanceof NotNode) {
            ...
            return node.clone().replaceTitle(headers, map, rows);
        }
...
GivenNode.prototype = {
    ...
    replaceTitle: function(headers, map, rows) {
        this.title = replaceText(this.title, headers, map, rows);
        return this;
    },
...
// Helpers --------------------------------------------------------------------
function replaceText(text, headers, map, rows) {
...
// Exports --------------------------------------------------------------------
// ----------------------------------------------------------------------------
module.exports = function(filename, language, keywords, outlines) {
    ...
    var l = new Lexer(language),
        p = new Parser(keywords || Keywords[language] || {}, outlines);
