    ...
    switch(error.type)  {
        ...
        case 'order':
            message += prefix + indent(value(context.getTitle()), 1)
                    + color(' is currently implemented as step ')
...
module.exports.header = function(prefix, title, level) {
    return indent(color(prefix + ':', Default) + ' ' + color(title, White), level || 0);
};
...
function keywords(string) {
    return string.replace(/(Test|Scenario|Feature|Step)/g, function(val) {
...
function location(loc, prefix) {
    ...
    if (loc.line !== -1 && loc.column !== -1) {
        return color(msg + ' (line ' + loc.line + ', column ' + loc.column + ')', Location);
