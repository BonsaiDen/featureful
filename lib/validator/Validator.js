...
Validator.prototype = {
    ...
    compare: function(specs) {
        ...
        specs.forEach(function(spec) {
            ...
            // Feature file without test files
            ...
            // Test file without feature files
            } else {
    ...
    _mergeSpec: function(spec, error) {
        ...
        if (features.length > 1) {
            ...
            features.slice(1).forEach(function(feature) {
                ...
                this._validateTestFeature(error, baseFeature, feature);
        ...
        if (tests.length > 1) {
            ...
            tests.slice(1).forEach(function(test) {
                ...
                // Validate against base feature
                this._validateTestScenario(error, baseFeature, test);
    ...
    _compareSpec: function(spec, error) {
        ...
        util.matchUp(
            ...
            function(feature, test) {
                ...
                this._validateTestScenario(error, feature, test);
            ...
            function(feature) {
                // TODO test or remove dead code
                spec.getFeatures().forEach(function(feature) {
                    error.addFeatureError(feature, {
                        type: 'missing',
                        message: 'Test implementation for feature is missing.',
                        expected: 'should be implementated in matching test file.',
                        actual: feature.getTitle(),
                        from: feature.getLocation(),
                        location: this._getTestLocation(feature.getLocation())
                    });
            ...
            function(test) {
                // TODO test or remvoe dead code
                spec.getTests().forEach(function(test) {
                    error.addFeatureError(test, {
                        type: 'missing',
                        message: 'Feature specification for test is missing.',
                        expected: 'should be implementated in matching feature file.',
                        actual: test.getTitle(),
                        from: test.getLocation(),
                        location: this._getFeatureLocation(test.getLocation())
                    }, this);
    ...
    _validateTestFeature: function(error, baseFeature, feature) {
    ...
    _validateTestScenario: function(error, feature, test) {
    ...
    _getTestLocation: function(loc) {
        if (this._options.matcher.type === 'path') {
            return {
            ...
            // TODO test locations in missing errors with non-path matchers
            return null;
        }
    },
