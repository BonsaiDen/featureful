...
Framework.prototype = {
    ...
    parse: function(filename, options) {
        ...
        ast.traverse(tree.program.body, function(node) {
            ...
            // Create a Test
            var test = new Test(
                describeNode.expression.arguments[0].value,
                describeNode.expression.arguments[0].value.trim(),
                getDescription(describeNode),
                getTags(describeNode, commentPrefix), {
                    filename: filename,
                    line: describeNode.expression.loc.start.line,
                    column: describeNode.expression.loc.start.column
                });
            ...
            ast.traverse(describeNode.expression.arguments[1].body, function(node) {
                ...
                var scenario = new Scenario(
                    itNode.expression.arguments[0].value,
                    itNode.expression.arguments[0].value.trim(),
                    getDescription(itNode),
                    getTags(itNode, commentPrefix), {
                        filename: filename,
                        line: itNode.expression.loc.start.line,
                        column: itNode.expression.loc.start.column
                    });
                    ...
                    ast.traverse(itNode.expression.arguments[1].body, function(node) {
                        ...
                        if (lineComment) {
                            scenario.addStep(new Step(lineComment, commentedNode, commentPrefix, {
                                ...
                                column: lineComment.loc.start.column
...
function getDescription(describeNode) {
    ...
    return descriptionNode ? descriptionNode.value : '';
